...
def defaults():
    ...
    env.code_dir = '/srv/redux_saga_workshop'
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.redux_saga_workshop.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.redux_saga_workshop.include",
                "remote_path": "/var/lib/docker-nginx/config/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.redux_saga_workshop.include",
                "remote_path": "/var/lib/docker-nginx/config/conf.d/%(filename)s"
            },
            {
                "default_site": True,
                "pattern": "%(target)s.conf",
                "filename": "redux_saga_workshop",
...
def deploy(id=None, silent=False, force=False, auto_nginx=True):
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/redux_saga_workshop')
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE redux_saga_workshop; '
         '      CREATE USER redux_saga_workshop WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE redux_saga_workshop to redux_saga_workshop;" '
         '| docker exec -i postgres-9.5 psql -U postgres'.format(db_password=db_password))

    # Upload local settings
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/redux_saga_workshop/settings/local.py', use_sudo=True)

    # Create log dir
    sudo('mkdir -p /var/log/redux_saga_workshop/')
    ...
    sudo('touch %s/redux_saga_workshop/app/webpack-stats.json' % env.code_dir)
    ...
    # Copy logrotate conf
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/redux_saga_workshop')
...
def collectstatic(npm_build=True):
    if npm_build:
        docker_compose_run('node', 'npm run build', name='redux_saga_workshop_npm_build')
...
def docker_compose_run(service, cmd='', name='redux_saga_workshop_tmp'):
